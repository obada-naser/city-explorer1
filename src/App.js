...
class App extends React.Component {
  ...
  weatherInfo = async () => {
    let newUrl = `${process.env.REACT_APP_SERVER_URL}/weather?searchQuery=Amman`;
    let weather = await axios.get(newUrl);

    this.setState({
      discription: weather.data.discription,
      date: weather.data.data
    })
    ...
    return (
      <>
        ...
        <Form onSubmit={this.gettingLocation}>
          ...
          <p className='name'>this is longitude:{this.state.long}</p>
          ...
          <Weather handleClick = {this.getWeatherInfo}
